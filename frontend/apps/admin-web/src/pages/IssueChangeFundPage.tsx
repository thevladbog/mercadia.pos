...
export function IssueChangeFundPage() {
  ...
  if (!activeStoreId) {
    return <div className="panel"><h1>{t('seniorCashier.changeFund')}</h1><p className="muted">{t('common.selectStore')}</p></div>;
  }
    ...
    <div className="panel">
      ...
      <section className="card">
        <form className="stack" onSubmit={handleSubmit}>
          ...
          <label>
            {t('seniorCashier.confirmBySenior')}{t('seniorCashier.idSuffix')}
            <input type="text" value={actorId} onChange={(e) => setActorId(e.target.value)} />
          </label>
          <label>
            {t('seniorCashier.confirmBySecondPerson')}{t('seniorCashier.idSuffix')}
            <input type="text" value={approvedById} onChange={(e) => setApprovedById(e.target.value)} />
          </label>
