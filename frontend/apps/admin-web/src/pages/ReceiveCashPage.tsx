...
import { createCashMovement, useListCashBalances, useListOpenStoreShifts } from '@mercadia/api-clients-store-edge';
...
export function ReceiveCashPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (response) => {
      if (response.status === 202) {
        ...
        navigate('/senior-cashier/dashboard', { state: { notice: t('seniorCashier.postedSuccessfully') } });
      }
    },
  ...
  if (!activeStoreId) {
    return <div className="panel"><h1>{t('seniorCashier.receiveCash')}</h1><p className="muted">{t('common.selectStore')}</p></div>;
  }
    ...
    <div className="panel">
      ...
      <section className="card">
        <form className="stack" onSubmit={handleSubmit}>
          <label>
            ...
            <select value={selectedDrawerId} onChange={(e) => setSelectedDrawerId(e.target.value)}>
              ...
              {drawerContainers.map((c) => (
                <option key={c.containerId} value={c.containerId}>{c.containerId}</option>
              ))}
            </select>
          </label>
          ...
          <label>
            {t('seniorCashier.expectedAmount')}{t('seniorCashier.kopecksSuffix')}
            <input type="number" value={expectedMinor} onChange={(e) => setExpectedMinor(e.target.value)} />
          </label>
          <fieldset>
          ...
          <label>{t('seniorCashier.confirmBySenior')}{t('seniorCashier.idSuffix')}<input type="text" value={actorId} onChange={(e) => setActorId(e.target.value)} /></label>
          <label>{t('seniorCashier.confirmBySecondPerson')}{t('seniorCashier.idSuffix')}<input type="text" value={approvedById} onChange={(e) => setApprovedById(e.target.value)} /></label>
